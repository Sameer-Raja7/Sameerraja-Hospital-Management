...
public class StaffServiceImpl implements StaffService {
    ...
    public void preloadAdminStaff() {
        ...
        String adminEmail = "vasamsettisameerraja@gmail.com";
        if (staffRepository.findById(adminEmail).isEmpty()) {
            // Create User
            User adminUser = new User();
            adminUser.setEmail(adminEmail);
            adminUser.setPassword(passwordEncoder.encode("sameer123"));
            adminUser.setRole("ROLE_SUPERADMIN");
            // Create Address (dummy address)
            Address adminAddress = new Address();
            adminAddress.setCity("Amalapuram");
            adminAddress.setState("Andhra pradesh");
            adminAddress.setCountry("India");
            adminAddress.setPostalCode("533201");
            adminAddress.setStreet("Sithapathirao peta");
            adminAddress.setLandMark("Bus stand");

            // Create Staff
            Staff adminStaff = new Staff();
            adminStaff.setFirstName("Sameer");
            adminStaff.setLastName("Raja");
            adminStaff.setPhoneNumber("9958585854");
